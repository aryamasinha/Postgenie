🧞 Postgenie – Your AI Assistant for Reels & Auto-Posting

Postgenie is an intelligent platform that helps creators, marketers, and brands effortlessly create short-form videos (Reels) using AI and auto-publish them to platforms like Instagram, LinkedIn, and more — all from a single unified dashboard.

No more switching between multiple tools. With Postgenie, content creation and distribution become as simple as making a wish.

✨ Key Features

🎬 AI Reel Generation
Generate engaging, branded Reels from text, scripts, or blog posts in seconds.

📅 Auto-Publishing
Schedule or instantly publish videos to Instagram (Business), LinkedIn, and Facebook.

🧠 Branding Made Easy
Auto-apply your logo, colors, font, and layout templates.

📈 Cross-Platform Analytics
View insights on reach, engagement, and performance — in one place.

👥 Team Collaboration
Share workflows, get feedback, and manage campaigns with your team.

🌟 Who Is It For?

Content creators & solopreneurs

Marketing agencies

Personal branding experts

Startup founders

Anyone managing multi-platform video content

🛠️ Tech Stack - TBD

Integrations  - TBD

🚀 Coming Soon

🎙️ AI voiceover and avatar integration

📲 Mobile App

🔁 Repurposing long-form videos into Reels

🧩 Plugin marketplace for templates and transitions

🔗 Get Started

👉 Visit https://postgenie-phi.vercel.app/

👉 Sign up for early access and be among the first to try AI-powered Reel creation and automation.

## License

This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details.
